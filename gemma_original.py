...
- If the user is asking for news, facts, general information, or something that can be found on the internet – use the `search` function. in answers also provide the date of the news.
...
- If the user is asking information about Cyber security center (Kiber xavfsizlik markazi), use the `test_excel_search` function to fetch relevant data. Provide the user's query and optionally a theme if known.
...
\`\`\`python
def test_excel_search(query:str): -> str:
    """
